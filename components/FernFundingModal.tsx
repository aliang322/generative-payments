...
export default function FernFundingModal({
  ...
  const [step, setStep] = useState<"loading" | "kyc" | "funding" | "success" | "error">("loading");
  ...
  const handleStartOnramp = async () => {
    try {
      ...
      if (result.kyc && result.kyc.customerId) {
        // Try to get KYC link
        try {
          const kycResponse = await fetch(`/api/fern/customer/${result.kyc.customerId}`);
          if (kycResponse.ok) {
            const kycData = await kycResponse.json();
            if (kycData.kycLink) {
              setKycLink(kycData.kycLink);
              setStep("kyc");
              return;
            }
          }
        } catch (kycError) {
          console.log("Could not fetch KYC link, proceeding with funding");
        }
      }

      // If no KYC needed or KYC link not available, proceed to funding
      setStep("funding");
    } catch (err) {
          ...
          {step === "kyc" && kycLink && (
            <div className="text-center py-8">
              <div className="mb-6">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">KYC Required</h3>
                <p className="text-gray-600 mb-6">
                  To fund your payment plan, you need to complete identity verification first.
                </p>
              </div>
